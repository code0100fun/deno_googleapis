...
/**
 * Security Token Service API Client for Deno
 * ==========================================
 * 
 * The Security Token Service exchanges Google or third-party credentials for a short-lived access token to Google Cloud resources.
 * 
 * Docs: http://cloud.google.com/iam/docs/workload-identity-federation
...
export class sts {
  ...
  constructor(client?: CredentialsClient, baseUrl: string = "https://sts.googleapis.com/") {
    this.#client = client;
   ...
   *
   */
  async v1Introspect(req: GoogleIdentityStsV1IntrospectTokenRequest): Promise<GoogleIdentityStsV1IntrospectTokenResponse> {
    const url = new URL(`${this.#baseUrl}v1/introspect`);
    ...
    return deserializeGoogleIdentityStsV1IntrospectTokenResponse(data);
  }
   ...
   *
   */
  async v1Oauthtoken(req: GoogleIdentityStsV1ExchangeOauthTokenRequest): Promise<GoogleIdentityStsV1ExchangeOauthTokenResponse> {
    const url = new URL(`${this.#baseUrl}v1/oauthtoken`);
    const body = JSON.stringify(req);
    const data = await request(url.href, {
      client: this.#client,
      method: "POST",
    ...
    return data as GoogleIdentityStsV1ExchangeOauthTokenResponse;
  }
   ...
   * method does not require the `Authorization` header, and using the header
   * can cause the request to fail.
   *
   */
  async v1Token(req: GoogleIdentityStsV1ExchangeTokenRequest): Promise<GoogleIdentityStsV1ExchangeTokenResponse> {
    ...
    return data as GoogleIdentityStsV1ExchangeTokenResponse;
  }
}
...
export interface GoogleIdentityStsV1betaOptions {
  ...
  /**
   * The intended audience(s) of the credential. The audience value(s) should
   * be the name(s) of services intended to receive the credential. Example:
   ...
   * is used to access Google APIs. The provided project overrides the project
...
export interface GoogleIdentityStsV1ExchangeOauthTokenRequest {
  /**
   * Optional. The client identifier for the OAuth 2.0 client that requested
   * the provided token. It is REQUIRED when the [client]
  ...
  clientId?: string;
  /**
   * Optional. The authorization code that was previously from workforce
   * identity federation's `authorize` endpoint. Required if the flow is
  ...
  grantType?: string;
  /**
   * Optional. redirect_url is required when the flow is authorization code
   * flow i.e. if grant_type is `authorization_code` See
  ...
  refreshToken?: string;
  /**
   * Optional. An optional list of scopes that are requested for the token to
   * be returned. See https://www.rfc-editor.org/rfc/rfc6749#section-3.3 Must be
   * a list of space-delimited, case-sensitive strings. Note: Currently, the
   * scopes in the request are not supported
   */
  scope?: string;
}
...
export interface GoogleIdentityStsV1ExchangeTokenRequest {
   ...
   * as a list of space-delimited, case-sensitive strings. Required when
   ...
   * `subject_token_type` to `urn:ietf:params:oauth:token-type:access_token`. If
   * an access token already contains security attributes, you cannot apply
...
export interface GoogleIdentityStsV1ExchangeTokenResponse {
   ...
   * access token. In this case, the access token has the same expiration time
   * as the `subject_token`.
   */
  expires_in?: number;
  /**
   * The token type. Always matches the value of `requested_token_type` from
   * the request.
   */
...
export interface GoogleIdentityStsV1IntrospectTokenRequest {
  /**
   * Required. The OAuth 2.0 security token issued by the Security Token
   * Service API.
   */
  token?: string;
  /**
...
export interface GoogleIdentityStsV1IntrospectTokenResponse {
  ...
  scope?: string;
  /**
   * The unique user ID associated with the provided token. For Google
   * Accounts, this value is based on the Google Account's user ID. For
   * federated identities, this value is based on the identity pool ID and the
   * value of the mapped `google.subject` attribute.
   */
  sub?: string;
  /**
   * The human-readable identifier for the token principal subject. For
   * example, if the provided token is associated with a workload identity pool,
   * this field contains a value in the following format:
...
function serializeGoogleIdentityStsV1IntrospectTokenResponse(data: any): GoogleIdentityStsV1IntrospectTokenResponse {
  return {
    ...data,
    exp: data["exp"] !== undefined ? String(data["exp"]) : undefined,
    iat: data["iat"] !== undefined ? String(data["iat"]) : undefined,
  };
}

function deserializeGoogleIdentityStsV1IntrospectTokenResponse(data: any): GoogleIdentityStsV1IntrospectTokenResponse {
  return {
    ...data,
    exp: data["exp"] !== undefined ? BigInt(data["exp"]) : undefined,
    iat: data["iat"] !== undefined ? BigInt(data["iat"]) : undefined,
  };
}
