...
/**
 * Groups Settings API Client for Deno
 * ===================================
 * 
 * Manages permission levels and related settings of a group.
 * 
 * Docs: https://developers.google.com/google-apps/groups-settings/get_started
...
export class GroupsSettings {
  ...
  constructor(client?: CredentialsClient, baseUrl: string = "https://www.googleapis.com/groups/v1/groups/") {
    this.#client = client;
  ...
  async groupsGet(groupUniqueId: string): Promise<Groups> {
    const url = new URL(`${this.#baseUrl}${ groupUniqueId }`);
...
export interface Groups {
   ...
   * organization can become members of this group. - false: Users not belonging
   ...
   * true, the whoCanPostMessage property is set to NONE_CAN_POST. - If reverted
   * from true to false, whoCanPostMessages is set to ALL_MANAGERS_CAN_POST. -
   * false: The group is active and can receive messages. - When false, updating
   * whoCanPostMessage to NONE_CAN_POST, results in an error.
   */
  archiveOnly?: string;
  /**
   ...
   * A group manager can't do this. When you change your group's address using
   * the Directory API or the control panel, you are changing the address your
   ...
   * individual users or new members do not apply to posts made on behalf of the
   * group. - false: Members can not post in behalf of the group's email
   * address.
   */
   ...
   * REPLY_TO_SENDER: The reply sent to author of message. - REPLY_TO_LIST: This
   * reply message is sent to the group. - REPLY_TO_OWNER: The reply is sent to
   * the owner(s) of the group. This does not include the group's managers. -
   * REPLY_TO_IGNORE: Group users individually decide where the message reply is
   * sent. - REPLY_TO_MANAGERS: This reply message is sent to the group's
   ...
   * sendMessageDenyNotification property being true. - false: When a message is
   ...
   * are: - true: All groups in the account are listed in the Groups directory.
   ...
   * to the moderation queue. This is the default. - SILENTLY_MODERATE: Send the
   * message to the moderation queue, but do not send notification to
   ...
   * domain can access your Google Groups service and view the list of groups in
   * your Groups directory. Warning: Group owners can add external addresses,
   ...
   * email to this group. - ALL_MEMBERS_CAN_VIEW: The group members can view the
   * group members list. - ALL_MANAGERS_CAN_VIEW: The group managers can view
   * group members list.
   */
